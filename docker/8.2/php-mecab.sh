...
if [ $HASMECAB ] ; then
    ...
    wget https://github.com/ranvis/php-mecab/archive/master.zip
    unzip master.zip
    cd php-mecab-master/mecab && phpize && ./configure --with-php-config=/usr/bin/php-config --with-mecab-config=/usr/bin/mecab-config && make && make install
    cd /etc/php/$SHORTVERSION/mods-available
    touch mecab.ini
    echo "extension=mecab.so" | tee -a mecab.ini
    phpenmod -v $SHORTVERSION mecab
